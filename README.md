...
> [!IMPORTANT]
> 本アプリケーションは現在、ローカル環境での実行を前提としたWebアプリケーションとして構成されている  
>しかし、コア機能をステートレスなAPIとして独立させているため、将来的にはクラウド環境へのデプロイも容易なアーキテクチャ設計である
...
この課題を解決するため、どのようなタイピングミスを犯しやすいのかを知り、その傾向に応じた練習問題を提供できるアプリを作成すればよいと考えた
...
| 文字列比較| 不採用| 文字抜けによる連鎖的なミスに対応できない |
...
加えて、ソフトウェア設計上の課題として、JavaFXにおける画面とビジネスロジックの密結合による状態管理の複雑化や、単体テストの実行が困難になるという問題が生じていた

そのため、配布・更新がしやすく、環境構築なしで利用でき、かつクライアントとバックエンドを分離できるSpring Boot版へ移行した
...
#### 設計方針
- 本アプリでは、書き込みが頻繁であることと複雑なクエリを使用する可能性を鑑みて、PostgreSQLを用いている
- ORMはSpring Data JPA
...
- ~~**再起動後も保持されるDBへの移行**~~: 2026/08/03 完了
- **過去結果の直接閲覧機能**
- **過去結果に基づく練習問題生成**
- **マルチユーザー対応**
